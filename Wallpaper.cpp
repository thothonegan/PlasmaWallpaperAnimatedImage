...
namespace
{
	...
	{
		static int s_area = KDebug::registerArea("PlasmaWallpaperAnimatedImage");
		return s_area;
	}
}
...
void Wallpaper::init (const KConfigGroup& config)
{
	...
	kDebug(debugArea()) << "init: " << m_filePath;
...
{
	kDebug(debugArea()) << "SETTINGS CHANGED";
	kDebug(debugArea()) << "Current File Path : " << m_filePath;
...
{
	kDebug(debugArea()) << "SET RENDER OPTIONS";
...
void Wallpaper::render (void)
{
	...
	if (m_renderOption == Wallpaper::TiledScaled)
	{
	...
	else if (m_renderOption == Wallpaper::Centered)
	{
